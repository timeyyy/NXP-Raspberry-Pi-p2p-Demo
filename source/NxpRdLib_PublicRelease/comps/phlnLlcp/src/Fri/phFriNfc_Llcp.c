...
static phNfc_sData_t * phFriNfc_Llcp_AllocateAndCopy(void *pOsal, phNfc_sData_t * pOrig)
{
   ...
   if (phOsal_GetMemory(pOsal, sizeof(phNfc_sData_t), (void *)&pDest) == PH_ERR_SUCCESS)
   {
      goto error;
   }
   if (phOsal_GetMemory(pOsal, pOrig->length, (void *)&(pDest->buffer)) == PH_ERR_SUCCESS)
   {
      goto error;
   }
...
error:
   if (pDest != NULL)
   {
      ...
      {
         phOsal_FreeMemory(pOsal, pDest->buffer);
      }
      phOsal_FreeMemory(pOsal, pDest);
   }
   return NULL;
}

static void phFriNfc_Llcp_Deallocate(void *pOsal, phNfc_sData_t * pData)
{
   if (pData != NULL)
   {
      if (pData->buffer != NULL)
      {
         phOsal_FreeMemory(pOsal, pData->buffer);
      }
      else
      {
         LLCP_PRINT("Warning, deallocating empty buffer");
      }
      phOsal_FreeMemory(pOsal, pData);
   }
}
...
static NFCSTATUS phFriNfc_Llcp_InternalDeactivate( phFriNfc_Llcp_t *Llcp )
{
       ...
       (Llcp->state == PHFRINFC_LLCP_STATE_ACTIVATION))
   {
      ...
      /* Stop timer */
	  phOsal_Timer_Stop(Llcp->osal, Llcp->hSymmTimer);

      /* Return delayed send operation in error, in any */
      if (Llcp->psSendInfo != NULL)
      {
          phFriNfc_Llcp_Deallocate(Llcp->osal, Llcp->psSendInfo);
                                                       ...
                                                       phNfc_sData_t    *psParamsTLV)
{
    ...
    /* Create the timer */
	phOsal_Timer_Create(Llcp->osal, &Llcp->hSymmTimer);
...
static void phFriNfc_Llcp_HandleMACLinkDeactivated( phFriNfc_Llcp_t  *Llcp )
{
   ...
   if (Llcp->hSymmTimer != PH_OSALNFC_INVALID_TIMER_ID)
   {
       phOsal_Timer_Delete(Llcp->osal, Llcp->hSymmTimer);
   }
...
static bool_t phFriNfc_Llcp_HandlePendingSend ( phFriNfc_Llcp_t *Llcp )
{
   ...
   if (bDeallocate)
   {
       phFriNfc_Llcp_Deallocate(Llcp->osal, psSendInfo);
   }
...
//   phFriNfc_LlcpMac_LinkStatus_CB_t
                              ...
                              void                             *pContext )
{
   ...
   else if (Llcp->state == PHFRINFC_LLCP_STATE_OPERATION_RECV)
   {
      ...
      Llcp->psSendInfo = phFriNfc_Llcp_AllocateAndCopy(Llcp->osal, psInfo);
      result = NFCSTATUS_PENDING;
   }
   else
   {
