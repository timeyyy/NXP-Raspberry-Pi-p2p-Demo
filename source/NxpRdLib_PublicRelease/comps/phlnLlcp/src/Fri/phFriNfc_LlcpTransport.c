         ...
         && !memcmp(pServiceName->buffer, pSocket->sServiceName.buffer, pServiceName->length))
      {
         ...
         for(cacheIndex=0;cacheIndex<PHFRINFC_LLCP_SDP_ADVERTISED_NB;cacheIndex++)
         {
            ...
            else
            {
               ...
               if (phOsal_GetMemory(psTransport->pLlcp->osal,
                   pServiceName->length, (void *)&(pCachedServiceName->sServiceName.buffer)) != PH_ERR_SUCCESS)
               {
   ...
   else
   {
      ...
      for(i=0;i<PHFRINFC_LLCP_NB_SOCKET_MAX;i++)
      {
         ...
         if (pLlcpTransport->pSocketTable[i].sServiceName.buffer != NULL) {
             phOsal_FreeMemory(pLlcp->osal, pLlcpTransport->pSocketTable[i].sServiceName.buffer);
         }
...
NFCSTATUS phFriNfc_LlcpTransport_CloseAll (phFriNfc_LlcpTransport_t *pLlcpTransport)
{
   ...
   for(i=0;i<PHFRINFC_LLCP_SDP_ADVERTISED_NB;i++)
   {
      ...
      if (pCachedServiceName->sServiceName.buffer != NULL)
      {
          phOsal_FreeMemory(pLlcpTransport->pLlcp->osal, pCachedServiceName->sServiceName.buffer);
      ...
      do
      {
         if(pLlcpTransport->pSocketTable[index].eSocket_State == phFriNfc_LlcpTransportSocket_eSocketDefault)
         {
            ...
            *pLlcpSocket = &pLlcpTransport->pSocketTable[index];
            ...
            if(psOptions != NULL)
            {
               // handle connection-oriented buffering
               if (eType == phFriNfc_LlcpTransport_eConnectionOriented)
               {
                  ...
                  {
                     uint8_t tmp1 = pLlcpTransport->pSocketTable[index].bufferSendMaxLength ;
                     uint8_t tmp2 = pLlcpTransport->pSocketTable[index].bufferRwMaxLength;
                     uint8_t tmp3 = psWorkingBuffer->length;
                     uint8_t tmp4 = pLlcpTransport->pSocketTable[index].bufferLinearLength;
                     uint8_t tmp5 = PHFRINFC_LLCP_MIU_DEFAULT;

                     pLlcpTransport->pSocketTable[index].pSocketErrCb(NULL, tmp1);
                     pLlcpTransport->pSocketTable[index].pSocketErrCb(NULL, tmp2);
                     pLlcpTransport->pSocketTable[index].pSocketErrCb(NULL, tmp3);
                     pLlcpTransport->pSocketTable[index].pSocketErrCb(NULL, tmp4);
                     pLlcpTransport->pSocketTable[index].pSocketErrCb(NULL, tmp5);
...
NFCSTATUS phFriNfc_LlcpTransport_Close(phFriNfc_LlcpTransport_Socket_t*   pLlcpSocket)
{
   ...
   LLCP_PRINT("phFriNfc_LlcpTransport_Close");
   ...
   {
      LLCP_DEBUG("\n Link Socket Type = %x",pLlcpSocket->eSocket_Type);
      switch(pLlcpSocket->eSocket_Type )
     {
        case phFriNfc_LlcpTransport_eConnectionOriented:
        {
           status = phFriNfc_LlcpTransport_ConnectionOriented_Close(pLlcpSocket);
           break;
        }
        case phFriNfc_LlcpTransport_eConnectionLess:
        {
                                                     ...
                                                     phNfc_sData_t                      *psServiceName)
{
   ...
   else
   {
      ...
      if (phOsal_GetMemory(pLlcpSocket->psTransport->pLlcp->osal, 
          psServiceName->length, (void *)(&pLlcpSocket->sServiceName.buffer)) != PH_ERR_SUCCESS)
      {
